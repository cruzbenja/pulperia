...
$sql = "SELECT v.id,v.fecha,v.cliente,v.total,count(vp.cantidad) as cantidadProductos,u.usuario,v.fechaEliminacion FROM venta as v 
LEFT join ventaproducto as vp on vp.idVenta = v.id
left join usuario as u on u.id = v.idUsuario
where v.estado = 'Eliminado'
GROUP by v.id,v.fecha,v.cliente,v.total,u.usuario,v.fechaEliminacion";
...
<div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Historial de Ventas Eliminadas</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="index.php">Inicio</a></li>
              <li class="breadcrumb-item"><a href="EliminarVentas.php">Eliminar Ventas</a></li>
                    ...
                    <?php 
                        ...
                        while ($row = $resultado->fetch_assoc()) { ?>
                          <tr>
                              ...
                              <button type='button' class='btn btn-primary btn-sm checkbox-toggle' onclick="VerMotivoEliminacion('<?php echo $row['id']; ?>')">Ver Motivo</button>
                              </td>                           
                          </tr>
                      <?php } ?>   
                    </tfooter>
...
<!--MODALS    -->
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              <form method="POST" id="formRegistroCasa" class="row g-3">             
                <div class="mb-3 row">
                  <label for="staticEmail" class="col-md-3 col-sm-3 col-form-label">Eliminado: </label>
  ...
  <script>
    ...
    function VerMotivoEliminacion(idVenta){
        $('#idVenta1').val(idVenta);

        $.ajax({
         url: '../clases/Cl_EliminarVentas.php?op=MotivoEliminacion',
         type: 'POST',
         data: {
          idVenta: idVenta
             }, 
    ...
    function Verusuario(idVenta){
        $('#idVenta1').val(idVenta);

        $.ajax({
         url: '../clases/Cl_EliminarVentas.php?op=verUsuario',
         type: 'POST',
         data: {
          idVenta: idVenta
             }, 
             success: function(vs) {
                 if (vs == 'error') {
                  Swal.fire("Error..!", "ha ocurrido un error al obtener el usuario que elimino la venta", "error");                     
                 }else{ 
                    $('#usuario').val(vs);       
                 }                  
             }
